  ...
  public class AddContactToInvisibleListTransaction : BaseSSITransaction
  {
    ...
    public override void OnComplete(DataTypes.SSIActionResultCode action)
    {
      switch (action) {
        case DataTypes.SSIActionResultCode.Success:
          ...
          break;
        default:
          throw new InvalidOperationException(string.Format("Cannot add contact '{0}' {1} to invisible list: {2}", Contact.Name, Contact.Identifier, action));
          break;
      }
    }
    ...
    public override void Validate()
    {
      if (Contact.DenyRecordItemId > 0)
        throw new InvalidOperationException("Cannot add a contact which is already member of the invisible list.");
    }
  }
}
